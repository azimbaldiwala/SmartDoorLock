...
checking = False  # Flag to check if the process should start
...
while True:
    ...
    # Listen for the 'a' key to start checking
    key = cv2.waitKey(1)
    if key == ord('a'):
        checking = True
    elif key == ord('q'):
    ...
    # Perform liveness and face recognition only if checking is True
    if checking:
        ...
        for (x, y, w, h) in faces:
            ...
            if preds > 0.5:
                ...
                # If liveness check succeeds, proceed with face recognition
                label = 'Real'
                ...
                for face_encoding, face_location in zip(face_encodings, face_locations):
                    ...
                    if name != "Unknown":
                        print("Door Unlocked!")
                        cv2.putText(frame, "Door Unlocked!", (left, top - 10), cv2.FONT_HERSHEY_DUPLEX, 0.5, (0, 255, 0), 1)
                        checking = False  # Stop checking after successful recognition
                        break
                
                if not face_recognized:
                    cv2.putText(frame, "Face Not Recognized!", (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
